...
class Tree(object):
    ...
    def print_tree(self, root):
        ...
        for curr in track:
            ...
            if curr_node.left:
                #current node becomes left, level goes down by 1, current left stays the same, current right becomes middle
                track.append((curr_node.left, curr_level + 1, curr_l, mid))
            #same for right part
            if curr_node.right:
                #current node becomes right, level goes down by 1, current left becomes middle, current right stays the same
